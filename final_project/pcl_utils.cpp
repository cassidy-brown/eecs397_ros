...
bool PclUtils::find_plane_fit(double x_min, double x_max, double y_min, double y_max, double z_min, double z_max, double dz_tol,
     Eigen::Vector3f &plane_normal, double &plane_dist, Eigen::Vector3f &major_axis, Eigen::Vector3f  &centroid, bool &isSquare) { 
    vector<int> indices;
    bool ans_valid = true;
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered (new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PassThrough<pcl::PointXYZ> pass; //create a pass-through object    
    pass.setInputCloud(pclTransformed_ptr_); //set the cloud we want to operate on--pass via a pointer
    pass.setFilterFieldName("x"); // we will "filter" based on points that lie within some range of x-value
    pass.setFilterLimits(x_min, x_max);
    pass.filter(*cloud_filtered); //fill this cloud with result
    int n_filtered = cloud_filtered->points.size();
    ROS_INFO("num x-filtered pts = %d",n_filtered);
    ...
    //pass.setIndices(inliers);
    pass.setInputCloud(cloud_filtered);
    pass.setFilterFieldName("y"); // we will "filter" based on points that lie within some range of y-value
    pass.setFilterLimits(y_min, y_max);
    pass.filter(*cloud_filtered);
    n_filtered = cloud_filtered->points.size();
    ROS_INFO("num y-filtered pts = %d",n_filtered);
    ...
    // modified from the project 6 lidar_tranformer
    float maxX = -1000;
    float minX = 1000;
    float maxY = -1000;
    float minY = 1000;
    ...
    //vec[2] = 0.0; //all pings in the frame are in x-y plane, so z-component is 0
    
    for (int i = 0; i < cloud_filtered->points.size(); i++) {
        float x = cloud_filtered->points[i].x; //consider the i'th point
        float y = cloud_filtered->points[i].y;
        float z = cloud_filtered->points[i].z;
            // ROS_WARN("(x,y,z)=(%6.3f, %6.3f, %6.3f)", x, y, z);

        Eigen::Vector3f currentPoint(x, y, z);

            // continually change the minimum and maximum values of x and y
            if (x < minX) {
                minX = x;
                left = currentPoint; //= cloud_filtered->points[i];
            }
            if (x > maxX) {
                maxX = x;
                right = currentPoint;
    ...
    // calculate the length of the x and y sides by subtracting the min from the max
    double difX = (right - down).norm();
    double difY = (right - up).norm();

    ROS_INFO("size of block = %f, %f", difX, difY);
    
    // ADD STUFF HERE!!!
    // This section should classify a block as a square or a rectangle
    // This function could simply take in another argument passed by reference where we set
    // some "isSquare" boolean to true if the object's side lengths are within a certain range 
    // of one another (to account for error)

    double sideProp = difX / difY;
    if(sideProp < 1){
        sideProp = 1 / sideProp;
    }

    isSquare = sideProp < 1.2;      // proportion between sides is less than 1.2. Gives lots of leeway for poor vision
    if(sideProp < 1.2){
        ROS_INFO("%f -- square-ish", sideProp);
    } else {
        ROS_INFO("%f -- not square", sideProp);

    }


    // // difference in side lengths
    // sideDif = difX - difY;
    // // absolute value
    // if (sideDif < 0) {
    //     sideDif = sideDif * -1;
...
/*void Find_width_and_height(Eigen::Affine3f A, float table_height) {

    for (int i = 0; i < pclTransformed_ptr_->points.size(); i++) {
        vec = pclTransformed_ptr_->points[i].getVector3fMap(); //consider the i'th point
        if (vec[2] < table_height) {
            ...
            // also check for maxZ for the midpoint
            if (vec[2] > maxZ) {
                maxZ = vec[2];
            }
        }
    ...
    // calculate the center point by taking the average of the min and max values
