...
bool ObjectFinder::find_toy_block(float surface_height, geometry_msgs::PoseStamped &object_pose) {
    ...
    found_object = pclUtils_.find_plane_fit(0.4, 1, -0.5, 0.5, surface_height + 0.025, surface_height + 0.045, 0.001,
            plane_normal, plane_dist, major_axis, centroid, isSquare);
    if(isSquare)
        result_.object_id = 1;
    else
        result_.object_id = 0;              //To avoid having to make change to ObjectIdCodes, because messing with learning_ros pkgs just complicates things
...
void ObjectFinder::executeCB(const actionlib::SimpleActionServer<object_finder::objectFinderAction>::GoalConstPtr& goal) {
    ...
    if (!known_surface_ht) {
        ...
        double table_ht;
        //hard-coded search range: x= [0.5,1], y= [-0.5,0.5], z=[-0.3,0.3] in steps of 0.005
